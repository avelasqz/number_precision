...
Line plot for lower-bounded events contexts:
``{r}
events_lower_tri_cat <- ggplot(coinCountsLowerEvents,aes(x=factor(delimit, level=c("below", "25", "above")), y=prop, group="delimit")) +
      geom_point() +
      geom_line() +
      labs(title="07_pilot events lower-bound contexts, by category", x="Value", y="Proportion")

events_lower_tri_cat
ggsave(plot = events_lower_tri_cat, filename = "07_pilot_events_lower_tri-cat.pdf", width = 5, height = 3, units = "in")
``

Line plot for upper-bounded events contexts:
``{r}
events_upper_tri_cat <- ggplot(coinCountsUpperEvents,aes(x=factor(delimit, level=c("below", "25", "above")), y=prop, group="delimit")) +
      geom_point() +
      geom_line() +
      labs(title="07_pilot events upper-bound contexts, by category", x="Value", y="Proportion")

events_upper_tri_cat
ggsave(plot = events_upper_tri_cat, filename = "07_pilot_events_upper_tri-cat.pdf", width = 5, height = 3, units = "in")
``

Layered plot for lower-bounded and upper-bounded events contexts:
``{r}
events_tri_cat_compare <- ggplot(coinCountsLowerEvents,aes(x=factor(delimit, level=c("below", "25", "above")), y=prop, fill="delimit", group="delimit")) +
      geom_point(color="blue") +
      geom_line(color="blue") +
      geom_point(data=coinCountsUpperEvents, color="red", show.legend = TRUE) +
      geom_line(data=coinCountsUpperEvents, color="red", linetype="33", show.legend = TRUE) +
      labs(title="07_pilot events lower- vs upper-bound, by category", x="Value", y="Proportion")

events_tri_cat_compare
ggsave(plot = events_tri_cat_compare, filename = "07_pilot_events_tri-cat_compare.pdf", width = 5, height = 3, units = "in")
``
...
Isolate events contexts and plot proportions--note: proportions slightly off here; the proportions were found by item-bound-macroCat grouping, then (probably) added together; the next chunk shows it done by grouping only by macroCat to begin with.
``{r}
p <- d_pivotedprops_item_bound_27 %>% 
...
p <- d_pivotedprops_item_bound_27 %>% 
        ...
        group_by(value) %>% 
        summarize(meanProp = mean(prop)) %>% 
        ggplot(aes(x=value,y=meanProp)) + 
        geom_bar(stat="identity") +
        labs(title="07_pilot events all contexts, n=318", x="Value", y="Proportion")

p
ggsave(plot = p, filename = "07_pilot_events_all2.pdf", width = 5, height = 3, units = "in")
``

Isolate events contexts and plot proportions--this is the more accurate way to do it.
``{r}
pivot_events_allv2 <- d_macroCat_25 %>%
                          filter(macroCat == "event") %>% 
                          ...
                          summarise("20" = mean(coin_a)/20,
                                    ...
                                    "29" = mean(coin_j)/20,
                                    "30" = mean(coin_k)/20) %>%
                          pivot_longer(cols = "20":"30", names_to = "value", values_to = "prop") %>%
                          mutate(triCat = case_when(
                            value %in% lowerProps ~ "below",
                            value %in% upperProps ~ "above",
                            value == "25" ~ "25")) %>%
...
pivot_events_allv2
ggsave(plot = pivot_events_allv2, filename = "07_pilot_events_all3.pdf", width = 5, height = 3, units = "in")
``

This equals the above--more accurate, but here because feeding in the correct pivoted data frame to begin with:
``{r}
p <- d_pivotedprops_macrocat_33 %>% 
        group_by(macroCat,value) %>% 
        filter(macroCat == "event") %>% 
        group_by(value) %>% 
        summarize(meanProp = mean(prop)) %>% 
...
p
ggsave(plot = p, filename = "07_pilot_events_all4.pdf", width = 5, height = 3, units = "in")
``


A check--this method gives the same proportions as that used to arrive at pivot_gamenight_lower:
``{r}
pivot_gamenight_lowerv2 <- d_pivotedprops_item_bound_27 %>% 
...
Visualize tri-category proportions by item and bound--a bit of a fiddly way:
``{r}
d_pivotedprops_item_bound_27 %>%
  mutate(val = case_when(
    triCat == "above" ~ 26,
    triCat == "below" ~ 24,
    triCat == "25" ~ 25
  )) %>% 
  group_by(triCat,val,item,bound) %>% 
  summarize(sumProp = sum(prop)) %>% 
  ggplot(aes(x=val,y=sumProp,color=item,linetype = bound)) + 
  geom_point() + 
  geom_line() +
  scale_x_continuous(breaks=c(24,25,26),
...
Model for 'below' as a function of bound:
```{r}
m_below_bound = lm(below ~ bound, data=d_triCatRaw_36)
summary(m_below_bound)
```

Model for 'below' as a function of bound and item:
```{r}
m_below_bound_item = lm(below ~ bound + item, data=d_triCatRaw_36)
summary(m_below_bound_item)
```

Model for 'below' as a function of bound and item, with interaction:
```{r}
m_below_bound_item_int = lm(below ~ bound + item + bound*item, data=d_triCatRaw_36)
summary(m_below_bound_item_int)
```

Model for 'exact' as a function of bound:
```{r}
m_25_bound = lm(exact ~ bound, data=d_triCatRaw_36)
summary(m_25_bound)
```

Model for 'exact' as a function of bound and item:
```{r}
m_25_bound_item = lm(exact ~ bound + item, data=d_triCatRaw_36)
summary(m_25_bound_item)
```

Model for 'exact' as a function of bound and item, with interaction:
```{r}
m_exact_bound_item_int = lm(exact ~ bound + item + bound*item, data=d_triCatRaw_36)
summary(m_exact_bound_item_int)
```

Model for 'above' as a function of bound:
```{r}
m_above_bound = lm(above ~ bound, data=d_triCatRaw_36)
summary(m_above_bound)
```

Model for 'above' as a function of bound and item:
```{r}
m_above_bound_item = lm(above ~ bound + item, data=d_triCatRaw_36)
summary(m_above_bound_item)
```

Model for 'above' as a function of bound and item, with interaction:
```{r}
m_above_bound_item_int = lm(above ~ bound + item + bound*item, data=d_triCatRaw_36)
summary(m_above_bound_item_int)
```
...
Model for bin as a function of bound and item, with interaction (is this a sensible model?):
```{r}
m_part_bin_by_item_bound = lm(n ~ bound + item + bound*item, data=part_bin_by_item_bound)
summary(m_part_bin_by_item_bound)
```
...
```{r}
m_mean_item = lmer(wgt_mean ~ bound + (1|workerid), data=d_raw_weighted_means_42)
summary(m_mean_item)
```
