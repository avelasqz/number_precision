...
```{r echo=FALSE, message=FALSE, warning=FALSE}
d <- read.csv("../../data/06_pilot/06_pilot-merged.csv")
summary(d)
#View(d)
```
...
lang_exclude = d %>%
  ...
  filter(str_detect(subject_information.language,regex("enligish", ignore_case=TRUE),negate=TRUE)) %>%
...
p <- ggplot(pivot_all_context) +
        ...
        labs(title="06_pilot all contexts, n=88", x="Value", y="Probability")

p
ggsave(plot = p, filename = "06_pilot_allcontexts.pdf", width = 5, height = 3, units = "in")
```
...
p <- ggplot(pivot_lowerbounds) +
      geom_col(aes(x=value, y=prob)) +
      labs(title="06_pilot lower-bound contexts, n=42", x="Value", y="Probability")

p
ggsave(plot = p, filename = "06_pilot_lowerbounds.pdf", width = 5, height = 3, units = "in")
```
...
p <- ggplot(pivot_upperbounds) +
      geom_col(aes(x=value, y=prob)) +
      labs(title="06_pilot upper-bound contexts, n=46", x="Value", y="Probability")

p
ggsave(plot = p, filename = "06_pilot_upperbounds.pdf", width = 5, height = 3, units = "in")
```
...
p <- ggplot(pivot_lower_all) +
      ...
      labs(title="06_pilot lower-bounded, individual trials, n=42", x="Value", y="Probability")

p
ggsave(plot = p, filename = "06_pilot_lowerbounds_individual.pdf", width = 15, height = 9, units = "in")
```
...
p <- ggplot(pivot_upper_all) +
      ...
      theme_bw() + 
      scale_fill_brewer(palette = "RdYlBu") +
      labs(title="06_pilot upper-bounded, individual trials, n=46", x="Value", y="Probability")

p
ggsave(plot = p, filename = "06_pilot_upperbounds_individual.pdf", width = 15, height = 10, units = "in")
```
